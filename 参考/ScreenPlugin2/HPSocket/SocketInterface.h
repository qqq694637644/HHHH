...
public:
	...
	* 参数：		pSender		-- 事件源对象
	*			dwConnID	-- 连接 ID
	* 返回值：	HR_IGNORE / HR_IGNORE	-- 继续执行
	*			HR_ERROR			-- 引发 OnClose() 事件并关闭连接
	*/
	virtual EnHandleResult OnHandShake(ITcpPackClient* pSender, CONNID dwConnID)												= 0;
	...
	*/
	virtual EnHandleResult OnSend(ITcpPackClient* pSender, CONNID dwConnID, const BYTE* pData, int iLength)					= 0;
	...
	*			HR_ERROR			-- 引发 OnClose() 事件并关闭连接
	*/
	virtual EnHandleResult OnReceive(ITcpPackClient* pSender, CONNID dwConnID, const BYTE* pData, int iLength)				= 0;
	...
	*			HR_ERROR			-- 引发 OnClose() 事件并关闭连接
	*/
	virtual EnHandleResult OnReceive(ITcpPackClient* pSender, CONNID dwConnID, int iLength)									= 0;
	...
	* 返回值：	忽略返回值
	*/
	virtual EnHandleResult OnClose(ITcpPackClient* pSender, CONNID dwConnID, EnSocketOperation enOperation, int iErrorCode)	= 0;
...
public:
	...
	* 返回值：	忽略返回值
	*/
	virtual EnHandleResult OnShutdown(ITcpPackClient* pSender)																= 0;
...
public:
	...
	*			HR_ERROR			-- 终止启动通信服务组件
	*/
	virtual EnHandleResult OnPrepareListen(ITcpPackClient* pSender, SOCKET soListen)						= 0;
	...
	* 返回值：	HR_IGNORE / HR_IGNORE	-- 接受连接
	*			HR_ERROR			-- 拒绝连接
	*/
	virtual EnHandleResult OnAccept(ITcpPackClient* pSender, CONNID dwConnID, UINT_PTR soClient)			= 0;
};
...
public:
	...
	*/
	virtual EnHandleResult OnPrepareConnect(ITcpPackClient* pSender, CONNID dwConnID, SOCKET socket)		= 0;
	...
	*								   异步连接：关闭连接
	*/
	virtual EnHandleResult OnConnect(ITcpPackClient* pSender, CONNID dwConnID)							= 0;
};
...
public:
	...
	*/
	virtual EnHttpParseResult OnMessageBegin(ITcpPackClient* pSender, CONNID dwConnID)										= 0;
	...
	*			HPR_ERROR	-- 引发 OnParserError() 和 OnClose() 事件并关闭连接
	*/
	virtual EnHttpParseResult OnRequestLine(ITcpPackClient* pSender, CONNID dwConnID, LPCSTR lpszMethod, LPCSTR lpszUrl)		= 0;
	...
	*			HPR_ERROR		-- 引发 OnParserError() 和 OnClose() 事件并关闭连接
	*/
	virtual EnHttpParseResult OnStatusLine(ITcpPackClient* pSender, CONNID dwConnID, USHORT usStatusCode, LPCSTR lpszDesc)	= 0;
	...
	*			HPR_ERROR	-- 引发 OnParserError() 和 OnClose() 事件并关闭连接
	*/
	virtual EnHttpParseResult OnHeader(ITcpPackClient* pSender, CONNID dwConnID, LPCSTR lpszName, LPCSTR lpszValue)			= 0;
	...
	*			HPR_ERROR		-- 引发 OnParserError() 和 OnClose() 事件并关闭连接
	*/
	virtual EnHttpParseResult OnHeadersComplete(ITcpPackClient* pSender, CONNID dwConnID)									= 0;
	...
	*			HPR_ERROR	-- 引发 OnParserError() 和 OnClose() 事件并关闭连接
	*/
	virtual EnHttpParseResult OnBody(ITcpPackClient* pSender, CONNID dwConnID, const BYTE* pData, int iLength)				= 0;
	...
	*			HPR_ERROR	-- 引发 OnParserError() 和 OnClose() 事件并关闭连接
	*/
	virtual EnHttpParseResult OnChunkHeader(ITcpPackClient* pSender, CONNID dwConnID, int iLength)							= 0;
	...
	*			HPR_ERROR	-- 引发 OnParserError() 和 OnClose() 事件并关闭连接
	*/
	virtual EnHttpParseResult OnChunkComplete(ITcpPackClient* pSender, CONNID dwConnID)										= 0;
	...
	*			HPR_ERROR	-- 引发 OnParserError() 和 OnClose() 事件并关闭连接
	*/
	virtual EnHttpParseResult OnMessageComplete(ITcpPackClient* pSender, CONNID dwConnID)									= 0;
	...
	*/
	virtual EnHttpParseResult OnUpgrade(ITcpPackClient* pSender, CONNID dwConnID, EnHttpUpgradeType enUpgradeType)			= 0;
	...
	*			HPR_ERROR		-- 引发 OnClose() 事件并关闭连接
	*/
	virtual EnHttpParseResult OnParseError(ITcpPackClient* pSender, CONNID dwConnID, int iErrorCode, LPCSTR lpszErrorDesc)	= 0;
	...
	*			HR_ERROR			-- 引发 OnClose() 事件并关闭连接
	*/
	virtual EnHandleResult OnWSMessageHeader(ITcpPackClient* pSender, CONNID dwConnID, BOOL bFinal, BYTE iReserved, BYTE iOperationCode, const BYTE lpszMask[4], ULONGLONG ullBodyLen)	= 0;
	...
	*			HR_ERROR			-- 引发 OnClose() 事件并关闭连接
	*/
	virtual EnHandleResult OnWSMessageBody(ITcpPackClient* pSender, CONNID dwConnID, const BYTE* pData, int iLength)			= 0;
	...
	*			HR_ERROR			-- 引发 OnClose() 事件并关闭连接
	*/
	virtual EnHandleResult OnWSMessageComplete(ITcpPackClient* pSender, CONNID dwConnID)										= 0;
